...
def summarize_result(action: str, r: dict) -> str:
    ...
    try:
        m = {
            "dataset_summary":           lambda: f"{len(r)} datasets",
            ...
            "gene_expression_by_group":  lambda: f"{len(r.get('result', {}))} genes in {r.get('dataset', '?')}",
            ...
            "gene_network_hub":          lambda: f"{r['interpretation']} | {r['n_edges']} edges",
            "cross_dataset_de":          lambda: f"{r['n_consistent_genes']} consistent genes across {len(r.get('datasets_used', []))} datasets",
